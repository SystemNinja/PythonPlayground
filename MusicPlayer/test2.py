...
layout = [
    ...
    [sg.Button("Previous"), sg.Button("Play", key="-PLAYSONG-"), sg.Button("Resume", key="-RESUMESONG-",visible=False), sg.Button("Pause"), sg.Button("Stop"), sg.Button("Next")]
]
...
while True:
    ...
    if event == "Play":
        player.play()
        window["-PLAYSONG-"].update(visible=False)
    if event == "Resume":
        player.resume()
    if event == "Pause":
        player.pause()

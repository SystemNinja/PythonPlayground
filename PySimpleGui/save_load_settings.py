...
"""
# path.dirname(__file__) -- Get current directory where python file is located
# path.join(path, r"settings_file.cfg") -- This will join file name with
SETTINGS_FILE = path.join(path.dirname(__file__), r"settings_file.cfg") # Get full path of the configuration file
...
def load_settings(settings_file, default_settings):
    try: # Try to open existing settings file
        with open(settings_file, 'r') as f:
            settings = jsonload((f)) #load json formatted file into python readable format
    except Exception as e: #If file not found, do following:
...
def save_settings(settings_file, settings, values):
    ...
    with open(settings_file, "w") as f:
        jsondump(settings, f) # convert data in python format to JSON format in a file
